...
1. Create a class `MyResultReceiver` to implement the `CCapturedResultReceiver` interface, and get the captured results in `OnCapturedResultReceived` callback function.
    ...
    >For the error handling mechanism, the SDK returns Error Code in the `CCapturedResult` object. You can add error handling code as needed. See [Error Code]({{site.enums}}core/error-code.html?src=cpp&&lang=cpp) for a full list of supported error codes.
